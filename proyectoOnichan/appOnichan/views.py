...
def pagina4(request):
    ...
    context['channels_json'] = json.dumps(context['channels'])
...
def dashboard_detail(request, chart, label):
    """Vista genérica que devuelve una página de detalle para un dato de un chart.
    `chart` puede ser: 'orders', 'products', 'revenue', 'channels'.
    """
    # Generar datos de ejemplo distintos según chart
    if chart == 'products':
        months = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic']
        base = sum(ord(c) for c in label) % 800
        values = [base + (i * 120) % 2000 + (len(label)*10) for i in range(len(months))]
        detail = {'label': label, 'months': months, 'values': values}
    elif chart == 'orders':
        hours = [f"{h}:00" for h in range(8,21)]
        values = [((sum(ord(c) for c in label) % 10) + i) * 5 + 10 for i in range(len(hours))]
        detail = {'label': label, 'labels': hours, 'values': values}
    elif chart == 'revenue':
        months = ['Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic']
        base = (sum(ord(c) for c in label) % 500) + 1000
        values = [base + i*200 for i in range(len(months))]
        detail = {'label': label, 'months': months, 'values': values}
    elif chart == 'channels':
        months = ['Ene','Feb','Mar','Abr','May','Jun']
        base = (sum(ord(c) for c in label) % 300)
        values = [base + i*50 + (len(label)%5)*20 for i in range(len(months))]
        detail = {'label': label, 'months': months, 'values': values}
    else:
        detail = {'label': label, 'months': ['A','B','C'], 'values': [1,2,3]}

    return render(request, 'dashboard_detail.html', {'detail': detail, 'detail_json': json.dumps(detail), 'chart': chart})
